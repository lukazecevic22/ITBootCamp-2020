...
brStranica = () => (radnici.length - 1) / poStrani;
stranice = (brStranica) => {
	let stranice = [];
    ...
    return (
        <>
            ...
            <div>
                {radnici.slice(stranica * poStrani, (stranice+1) * poStrani).map(worker => <Radnik worker={worker} key={worker} />)}
            </div>
            <div>
            {stranice(brStranica()).map(stranica => <button onclick={()=>setStranica(stranica)}>{stranica+1}</button>)}
            </div>
        </>
    )
